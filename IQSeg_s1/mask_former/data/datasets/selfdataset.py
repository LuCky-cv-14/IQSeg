...
class  MSRSDataset(Dataset):
    def __init__(self, root_dir=r'/space1/liuchang/VI/SegMiF-main/MSRS_Dataset/test', is_train= True):
        ...
        if is_train== True:
            self.augs0 = T.AugmentationList([
                                            T.ResizeShortestEdge([256, 340, 440, 540, 600], 2560, "choice"),
                                            T.RandomCrop_CategoryAreaConstraint(crop_type="absolute", crop_size = [256, 256]),
        ...
        # else:
        #     points_per_side = 16
        # self.mask_generator = load_sam(points_per_side)
    ...
    def __getitem__(self, idx):
        ...
        # start = time.time()
        # sam_masks = self.mask_generator.generate(image_)
        # sam_masks_vi = self.mask_generator.generate(image_vi_)
        # end = time.time()
        # print("time:", end - start)

        im_size = image_.shape[:2]
        # ps_ = probs_process(sam_masks)
        # ps_vi_ = probs_process(sam_masks_vi)

        # check input
        # save_img(image_, image_vi_, sem_seg_gt_, sam_masks, sam_masks_vi)
        # save_all(image, sem_seg_gt, ps)
        ...
        # ps = torch.tensor(np.array(ps_)).permute(2, 0, 1) 
        # ps_vi = torch.tensor(np.array(ps_vi_)).permute(2, 0, 1) # 3, 480, 640
        sem_seg_gt = torch.tensor(np.array(sem_seg_gt_))

        # check input
        # save_img(image, sem_seg_gt, sam_masks)
        # save_all(image_, sem_seg_gt_, ps)        
