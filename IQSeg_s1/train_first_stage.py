...
class Trainer(DefaultTrainer):
    ...
    def build_train_loader(cls, cfg):
        ...
        # DETR-style dataset mapper for COCO panoptic segmentation
            ...
            mapper = None
            dataset = MSRSDataset(cfg.TRAIN_PATH, True)
        #TODO build_detection_train_loader 可直接传递
...
def setup(args,i):
    ...
    if i is None:
        add_path_config_train(cfg)
    else:
        add_path_config_test(cfg,i)
...
def test_all_models(args):
    sqmodelname=[44999]
    # sqmodelname=[14999, 19999,24999,29999,34999,39999,44999,49999,54999,59999]
    for i in sqmodelname:
        cfg = setup(args, i)
        model = Trainer.build_model(cfg)
        DetectionCheckpointer(model, save_dir=cfg.OUTPUT_DIR).resume_or_load(cfg.MODEL.WEIGHTS, resume=args.resume)
        #! 原始测试
...
def cal_param(args):
    ...
    for i in sqmodelname:
        ...
        input = {
            ...
            "image_vi": torch.randn(3, 480, 640),
        }
...
def cal_param1(args):
    ...
    for i in sqmodelname:
        ...
        print(f'total_params: {total_params/10000}, total_trainable_params: {total_trainable_params}')

def add_path_config_train(cfg):
    cfg.OUTPUT_DIR = f"xxx/IQSeg/IQSeg_s1/checkpoint/FMB_r50"
...
def add_path_config_test(cfg, i):
    cfg.OUTPUT_DIR = f"xxx/IQSeg/IQSeg_s1/output/eval_FMB_first_stage"
    cfg.MODEL.WEIGHTS = f"xxx/IQSeg/checkpoint/FMB_first_stage_r50.pth"
    cfg.TRAIN_PATH = "xxx/RGBTdataset/FMB_Dataset/train"
    cfg.TEST_PATH = "xxx/RGBTdataset/FMB_Dataset/test"
...
if __name__ == "__main__":
    ...
    launch(
        test_all_models,  #! test: test_all_models; train: train
        args.num_gpus,
